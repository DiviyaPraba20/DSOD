...
export class DSODSponsorCategoriesTabsCompnonent implements OnInit, OnDestroy {
  ...
  ngOnInit(){
    ...
    this.filterTabs$ = this.store.select(state => state.cms.sponsoredCategories)
  }

  fetchSponsorCategories(){
    this.storeSub= this.store.select(state => state.cms.sponsorsList).pipe(takeUntil(this._unsubscribeAll)).subscribe(items => {
    if (items.length) {
      const sponsor = items.filter(item => item.name == this.sponsorName);
      this.extractedId = sponsor[0].id;
      this.store.dispatch(new actions.FetchSponsoredCategories(sponsor[0].id))
    }
    })
  }
